# NOTE: the spec file name must stay "<Name>.spec" (tuxbench-kmod.spec).
...
%global kmod_name tuxbench

# Upstream module version (matches src/tuxbench/dkms.conf); bump only on an
...
Release:        1%{?dist}
Summary:        Kernel-mode memory latency/bandwidth benchmark for QTuxTimings
...
%description
Character-device kernel module that runs AVX2 memory latency and bandwidth
benchmarks for the QTuxTimings telemetry viewer. The module Makefile carries
the required -mavx2 flag.
